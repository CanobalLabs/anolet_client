...
(async () => {
  ...
  wss.on("connection", async (ws, req, locals) => {
    ...
    if (process.env.ENVIRONMENT != "dev") {
      axios.get(
        process.env.BASE_URL + "/ACCService/" + ws.game + "/increaseVisitCount",
        {
          headers: {
    ...
    axios
      .get(process.env.BASE_URL + "/game/" + locals.game, {
        headers: {
          serverauth: process.env.HASH,
        },
      })
      .then(async (res) => {
        ...
        axios
          .get(process.env.BASE_URL + "/user/" + locals.user)
          .then(async (user) => {
            await client.hSet("player:" + locals.game + ":" + locals.user, [
              "avatar",
              user.data.defaultRender
                ? "https://cdn.anolet.com/avatars/anolet/internal.png"
                : "https://cdn.anolet.com/avatars/" +
                  locals.user +
                  "/internal.png",
              "username",
              user.data.username,
              "x",
              res.data.worldSettings.spawn.x,
              "y",
              res.data.worldSettings.spawn.y,
              "id",
              locals.user,
              "admin",
              user.data.ranks.includes("ADMIN_TAG"),
            ]);
            ...
            pubsub.broadcast(
              ...
              JSON.stringify({
                type: "newplr",
                avatar: user.data.defaultRender
                  ? "https://cdn.anolet.com/avatars/anolet/internal.png"
                  : "https://cdn.anolet.com/avatars/" +
                    locals.user +
                    "/internal.png",
    ...
    ws.on("close", async (reason) => {
      ...
      require("./utils/deleteUser")(locals.game, locals.user, currentGames);
    });
        ...
        response.data.forEach(async function (game) {
          axios.get(
            process.env.BASE_URL +
              ...
              game.id +
              "/setPlayerCount/" +
              (await client.sCard("players:" + game.id)),
            {
              headers: {
                serverauth: process.env.HASH,
              },
